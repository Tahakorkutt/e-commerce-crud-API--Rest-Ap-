...
const resetPassword = async (req, res) => {
  ...
  try {
    ...
    // Reset password alanlarını temizle
    user.resetPasswordToken = undefined;
    user.resetPasswordExpire = undefined;
...
const userDetail = async (req, res, next) => {
  try {
    ...
    console.error("Kullanıcı detayı alınırken bir hata oluştu:", error);
    res.status(500).json({ message: "Kullanıcı detayı alınamadı" });
  }
};

const userDetailUpdate = async (req, res, next) => {
  try {
    ...
    console.error("Kullanıcı detay güncellenirken bir hata oluştu:", error);
    res.status(500).json({ message: "Kullanıcı detayı güncellenemedi" });
  }
};
Bu fonksiyon, User.findByIdAndUpdate ile kullanıcının detaylarını günceller. Güncelleme başarılıysa güncellenmiş kullanıcı bilgilerini döner, aksi takdirde uygun bir hata mesajı döner.
...
module.exports = { register, forgotPassword, resetPassword, login, logout, updatePassword,userDetail };
